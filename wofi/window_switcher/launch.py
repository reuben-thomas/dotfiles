...
# -*- coding: utf-8 -*-
# Code from
# https://github.com/quangdinh/swaywm-scripts/tree/main
...
def get_cons(tree):
    if (
        len(tree.get("nodes")) == 0
        and len(tree.get("floating_nodes")) == 0
        and (tree.get("type") == "con" or tree.get("type") == "floating_con")
    ):
        return [tree]
    nodes = []
...
def show_wofi(windows):
    command = (
        "wofi -W 600 -L "
        + str(len(windows))
        + ' -I -d -i -b -k /dev/null -p "Window" 2>/dev/null'
    )
...
def get_windows():
    ...
    for con in cons:
        ...
        windows.append(
            "img:id=" + str(con.get("id")) + ":img:" + app_icon + ":text:" + name
        )
    return windows
...
if __name__ == "__main__":
    ...
    icon_names = {"code": "visual-studio-code", "qt5ct": "preferences-desktop-theme"}
